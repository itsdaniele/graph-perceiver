...
class Attention(nn.Module):
    def __init__(
        self, *, dim, heads=8, dim_head=64, dropout=0.0, xl_max_memories=0.0,
    ):
        super().__init__()
        self.heads = heads
...
class MemorizingTransformer(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.to_logits = nn.Sequential(nn.LayerNorm(dim), nn.Linear(dim, num_logits))
    ...
    def forward(
        self, x, knn_memories, xl_memories=None, labels=None, add_knn_memory=True
    ):
        ...
        for ind, (attn, ff) in enumerate(self.layers):
            ...
            #     if not is_memorizing_layer
            #     else knn_rel_pos_bias
            # )
