...
from torch_geometric.nn import GCNConv as GCNConvGeometric, SAGEConv, GATConv, NNConv
...
class NNCONVPROTEINS(torch.nn.Module):
    def __init__(self, emb_dim):
        super().__init__()

        emb_dim = 32

        self.embed_node = torch.nn.Embedding(8, 8)
        self.conv1 = NNConv(8, emb_dim, nn=torch.nn.Linear(8, emb_dim * 8))
        self.conv2 = NNConv(
            emb_dim, emb_dim, nn=torch.nn.Linear(emb_dim * 8, emb_dim * emb_dim)
        )
        self.conv3 = NNConv(
            emb_dim, 256, nn=torch.nn.Linear(emb_dim * emb_dim, emb_dim * 256)
        )
        # self.conv3 = SAGEConv(emb_dim, 112)

    def forward(self, data, training=False):
        x, adj_t = data.node_species, data.adj_t

        x = self.embed_node(x.squeeze(-1))

        x = self.conv1(x, adj_t, data.edge_attr)
        x = F.relu(x)
        # x = F.dropout(x, p=0.5, training=training)
        x = self.conv2(x, adj_t, data.edge_attr)
        x = F.relu(x)
        # x = F.dropout(x, p=0.5, training=training)
        x = self.conv3(x, adj_t, data.edge_attr)
...
class GCNCHAM(torch.nn.Module):
    def __init__(self, emb_dim):
        super().__init__()
        self.conv1 = GCNConvGeometric(2089, emb_dim)
        self.conv2 = GCNConvGeometric(emb_dim, 5)
        self.conv3 = GCNConvGeometric(emb_dim, 5)
    ...
    def forward(self, data):
        ...
        # edge_index, _ = dropout_adj(edge_index, p=0.7, training=training)
...
class GNN_node_Virtualnode_no_batchnorm(torch.nn.Module):
    ...
    def __init__(
        ...
        super(GNN_node_Virtualnode_no_batchnorm, self).__init__()
        self.num_layer = num_layer
        self.drop_ratio = drop_ratio
        self.JK = JK
        ### add residual connection or not
    ...
    def forward(self, batched_data, perturb=None, training=None):
        ...
        edge_index, _ = dropout_adj(
            edge_index, p=0.3, training=training, force_undirected=True
        )
        node_depth = (
            batched_data.node_depth if hasattr(batched_data, "node_depth") else None
        )
